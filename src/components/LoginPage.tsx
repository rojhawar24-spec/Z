...
export default function LoginPage() {
  ...
  /* ── Google login (simpele versie) ── */
  const handleGoogle = () => {
    setError("");
    setGoogleLoading(true);
    setTimeout(() => {
      try {
        // Eenvoudige Google login: elke unieke naam krijgt een account
        // Als je terugkomt met dezelfde naam+email, log je weer in
        const uniek = "google_user_" + Date.now();
  ...
  const features = [
    ...
    { emoji: "📝", text: { nl:"Toetsen", en:"Tests", fr:"Tests", es:"Tests", de:"Tests" }[language] },
            ...
            <form onSubmit={handleEmailAuth} className="space-y-3">
              {isRegister && (
                <div className="relative">
                  ...
                  <input type="text" placeholder={t.nameLabel} value={name} onChange={e => setName(e.target.value)} autoComplete="name"
              ...
              <div className="relative">
                ...
                <input type="email" placeholder={t.emailLabel} value={email} onChange={e => setEmail(e.target.value)} autoComplete="email"
                  className={`w-full pl-11 pr-4 py-3.5 rounded-2xl border text-sm transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500/30 ${isDark ? "bg-gray-900 border-gray-800 text-white placeholder-gray-600 focus:border-indigo-600" : "bg-white border-gray-200 text-gray-900 placeholder-gray-400 focus:border-indigo-400"}`}
                />
              </div>
              <div className="relative">
                <Lock className={`absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 ${isDark ? "text-gray-600" : "text-gray-400"}`} />
                <input type={showPw ? "text" : "password"} placeholder={t.passwordLabel} value={password} onChange={e => setPassword(e.target.value)} autoComplete={isRegister ? "new-password" : "current-password"}
